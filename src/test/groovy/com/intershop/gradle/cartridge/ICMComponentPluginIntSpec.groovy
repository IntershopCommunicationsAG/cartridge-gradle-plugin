...
class ICMComponentPluginIntSpec extends AbstractIntegrationSpec {
    ...
    def 'Test simple component build - #gradleVersion'(gradleVersion) {
        ...
        File cartridgeZip = new File(testProjectDir, "build/${IntershopExtension.MAIN_OUTPUTDIR_NAME}/cartridge/${projectName}-cartridge.zip")
        File shareZip = new File(testProjectDir, "build/${IntershopExtension.MAIN_OUTPUTDIR_NAME}/share/${projectName}-share.zip")
    ...
    def 'Test publish component build - ivy happy path - #gradleVersion'(gradleVersion) {
        ...
        File cartridgeZip = new File(testProjectDir, "${pluginBuildDir}/cartridge/${projectName}-cartridge.zip")
        File shareZip = new File(testProjectDir, "${pluginBuildDir}/share/${projectName}-share.zip")
        ...
        ivyFile.text.contains('<artifact ext="jar" name="testfile" type="jar"/>')
